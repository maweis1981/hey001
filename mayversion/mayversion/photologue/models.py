...
class Gallery(models.Model):
    ...
    user = models.ForeignKey(User,related_name='my_gallery')
    ...
    def admin_thumbnail(self):
        ...
        if func is None:
            ...
            if hasattr(self, 'get_absolute_url'):
                ...
                return u'<a href="%s"><img src="%s"></a>' % \
                    (self.image.url, func())
    admin_thumbnail.short_description = _('Admin Thumbnail')
    admin_thumbnail.allow_tags = True

    def thumbnail(self):
        func = getattr(self, 'get_thumbnail_url', None)
        if func is None:
            return _('An "thumbnail" photo size has not been defined.')
        else:
            if hasattr(self, 'get_absolute_url'):
                return u'<a href="%s"><img src="%s"></a>' % \
                    (self.get_absolute_url(), func())
...
class Photo(ImageModel):
    title = models.CharField(_('title'), max_length=100, unique=True)
    user = models.ForeignKey(User,related_name='my_photoes')
