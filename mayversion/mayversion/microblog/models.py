...
class TweetInstanceManager(models.Manager):
    ...
    def tweets_for(self, recipient):
        ...
        return TweetInstance.objects.filter(recipient_type=recipient_type, recipient_id=recipient.id).order_by("-sent").select_related(depth=1)[:20]
        
    def public_tweets(self):
        return TweetInstance.objects.all().order_by("-sent")[:20]
